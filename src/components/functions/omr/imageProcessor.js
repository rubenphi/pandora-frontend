...
export function initOpenCV(OMR_STATE) {
  ...
  // fetchMatrixCode(OMR_STATE); // Asynchronously fetch and parse the matrix code - This will be handled by main.js

  processingLoop(OMR_STATE); // Start the main loop
}

export function processingLoop(OMR_STATE) {
  ...
  if (markers.length >= 6) {
    ...
    if (allMarkersPresent) {
      ...
      handleCapture(OMR_STATE, markers);
    }
  }
  ...
  requestAnimationFrame(() => processingLoop(OMR_STATE));
}

export function findAndLabelMarkers(
  OMR_STATE,
  contours,
  hierarchy,
  width,
...
export function drawMarkerOverlays(
  OMR_STATE,
  dst,
  markers,
  contours,
...
export function drawCodeMatrix(OMR_STATE, ctx, markers) {
  const labelMap = {};
  markers.forEach((m) => {
...
export function performPerspectiveCorrection(OMR_STATE, snapCanvas, labelMap) {
  const cv = getCv();
...
export function extractAndDrawResults(
  OMR_STATE,
  correctedCanvas,
  correctedMat,
  matrix,
