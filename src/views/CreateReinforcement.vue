    ...
    <ion-header>
      <ion-toolbar>
        <ion-buttons slot="start">
          <ion-button
            :href="`/refuerzos/${curso}/${area}/${periodo}/${year}`"
          >
        ...
        <ion-title>{{ isEditing ? 'Editar Refuerzo' : 'Crear Refuerzo' }}</ion-title>
      </ion-toolbar>
    </ion-header>
    <ion-content>
      <ion-list>
        ...
        <ion-item v-for="student in studentList" :key="student.id">
            ...
            <ion-label>
                <h2>{{ student.name }}</h2>
                <p :class="getAverageColor(student.average)">Promedio: {{ student.average ? student.average.toFixed(2) : 'N/A' }}</p>
            </ion-label>
        </ion-item>
  ...
  setup() {
    ...
    onIonViewWillEnter(async () => {
      ...
      try {
          ...
          const promises = [
              axios.get(`/grades?year=${year}&courseId=${curso}&areaId=${area}&periodId=${periodo}&instituteId=${usuario.value.institute.id}`),
              axios.get(`/courses/${curso}/users?year=${year}`)
          ];
    ...
    const crearRefuerzo = async () => {
        ...
        const selectedStudentIds = studentList.value.filter(s => s.selected).map(s => s.id);
    ...
    return {
        ...
        studentList,
        crearRefuerzo,
        curso, area, year, periodo,
        arrowBackOutline, checkmarkOutline,
        error, isLoadingStudents,
        getAverageColor,
        isEditing
    };
  }
